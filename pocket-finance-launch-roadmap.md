...
| ✅ Check Twelve Data + Massive.com billing | You | Confirmed both were dead integrations — removed ~5,900 lines of unused code (components, API routes, lib files) tied to them. Massive had no payment method on file. Twelve Data's Venture plan ($149/mo) has been cancelled. |
| ✅ Remove NewsAPI | Me | Was wired in as a secondary article source on top of Marketaux, hitting its free-tier rate limit (100 req/day, non-commercial only) during every build. Now on Marketaux exclusively — it's returning full 50-article pages on the Standard plan, so the NewsAPI top-up isn't needed. Removed the integration plus its now-dead filtering code (~500 lines) and updated the Privacy Policy/Terms pages that named it as a data source. `NEWS_API_KEY` can be deleted from Vercel's environment variables. |
...
| ✅ Fix Marketaux build-time timeouts | Me | The shared `/home` layout was firing duplicate Marketaux requests once per statically generated route (7 routes × 2 calls = 14 near-simultaneous requests), overwhelming the fetch timeout. Fixed with a module-level single-flight dedup in `fetchNews.ts` (14 calls → 2) plus a bump from 8s to 20s. One occasional slow response can still show up in build logs (one specific trending query is just sometimes slow on Marketaux's end) — non-blocking, build succeeds and deploys regardless, left as-is. |
| ✅ Fix feed re-sort thrashing found in live simulation testing | Me | An 85-card live scroll test (see `pocket-finance-live-simulation-report.md`) found articles resurfacing and infinite scroll never firing. Root cause: `ArticlePanel`/`BusinessInfoPanel` are always mounted in the background for the centered feed card (for the swipe transition), and were unconditionally firing "read"/"panel opened" tracking on every scroll — each one forces a full personalization re-sort. Gated both behind an `active` prop that's only true once the panel is actually swiped open. Also hardened infinite scroll to retry across a few pages instead of permanently disabling on one empty/overlapping response, debounced the Supabase stories-read counter (was doing a SELECT+UPSERT per scroll), broadened the placeholder-image detection, fixed a truncated-description-with-no-ellipsis bug, and deprioritized dry wire/PR boilerplate (SEC 8.3 filings, proxy statements, conference-call invites) in feed ranking. |
| ✅ Root-cause and fix infinite scroll for real | Me | A further live round confirmed the earlier fix hadn't fully solved it — network capture showed the fetch firing and succeeding every time, but never appending anything. Actual cause: `loadingMoreArticles`/`hasMoreArticlePages`/`nextFeedPage` were `useState` *and* listed in the fetch effect's own dependency array, so the effect's first action (`setLoadingMoreArticles(true)`) changed one of its own deps, re-triggering the effect and cancelling its own in-flight fetch before the results could be used. Converted all three to `useRef` (none are read in JSX). Verified live: card count now genuinely grows on scroll and the page counter advances correctly. |
| ✅ Fix exchange/market mislabeling | Me | Live testing found NASDAQ/NYSE tags on clearly non-US stories (a Nifty/Sensex India story tagged "NASDAQ: Micron", an explicit "(ASX:REH)(ASX:REA)" headline tagged "NYSE", a rupee/crude story tagged "NYSE: Citigroup"). Cause: Marketaux's single highest-matchScore entity can be a passing mention, not the story's subject, and that entity's home exchange overrode everything. Added an explicit "(EXCHANGE:TICKER)"-in-headline override (strongest possible signal) plus a headline-country-vs-entity-country mismatch check that discards a passing-mention entity when the headline clearly reads as e.g. an India/Japan/Korea/Hong Kong market story. Verified against legitimate cases (AT&T, Reliance) to confirm no regression. |
| ✅ Broaden non-finance PR filtering + fix mangled ticker symbols | Me | A marketing-conference press release ("Campaign Connect Indonesia") was landing in the feed with a real market tag despite having nothing to do with finance — added a non-finance-PR keyword list and a soft penalty for known low-quality wire-republishing sources. Separately, a JPMorgan article displayed ticker "JPMMV" instead of "JPM" (Marketaux returned a non-standard symbol variant) — added a company-name reverse lookup so a known company's clean ticker is used instead of the raw variant. |
